...
const AlmacenesEditar = () => {
  const [isDark] = useDarkMode();
  const [nombre, setNombre] = useState();
      
  const id = localStorage.getItem("EditStorage");

  
  const getStorage = async () => {
    try {
      const res = await clienteAxios.get("/almacen/single/"+id);
  ...
  const sendData = (event) => {
    ...
    if(nombre == "" || nombre == undefined) {
      ...
      const editStorage = async () => {
        try {
          const res = await clienteAxios.put("/almacen/actualizar", {
            id:id,
            nombre 
          });
          //console.log(res);
...
export default AlmacenesEditar;
