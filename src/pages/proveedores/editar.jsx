...
const ProveedoresEditar = () => {
  ...
  const id = localStorage.getItem("EditSupplier");

  
  const getSupplier = async () => {
    try {
      const res = await clienteAxios.get("/proveedor/single/"+id);
  ...
  const sendData = (event) => {
    ...
    if(nombre == "" || nombre == undefined) {
      ...
      const editSupplier = async () => {
        try {
          const res = await clienteAxios.put("/proveedor/actualizar", {
            id:id,
            nombre 
          });
          //console.log(res);
